...
## HTML 字符实体
  ...
  - [16.1](#16.1) **HTML 实体**
    ...
    + 🎈 在 [HTML](#no-jump) 中，某些字符是预留的。
    
    + 🎈 在 [HTML](#no-jump) 中不能使用小于号（[<](#no-jump)）和大于号（[>](#no-jump)），这是因为浏览器会误认为它们是标签。
    
    + 🎈 如果希望正确地显示预留字符，我们必须在 [HTML](#no-jump) 源代码中使用字符实体（character entities）。 字符实体类似这样：小于号（[<](#no-jump)） 为 [&lt;](#no-jump) 或 [&#60;](#no-jump) 或 [&#060;](#no-jump)
      
  <a name="16.2"></a>
  - [16.2](#16.2) **不间断空格(Non-breaking Space)**
  
    + 🚀 [HTML](#no-jump) 中的常用字符实体是不间断空格(&nbsp;)。
    
    + 🚀 浏览器总是会截短 [HTML](#no-jump) 页面中的空格。如果您在文本中写 10 个空格，在显示该页面之前，浏览器会删除它们中的 9 个。如需在页面中增加空格的数量，您需要使用 [&nbsp;](#no-jump) 字符实体。
